"""
Single-PCA epoch drift Phase-0 worker — fits ONE global preprocessing bundle
(standardize → PCA → standardize) on ALL words from ALL 2-hour epochs across
ALL recording days for a patient.

Source run uses nested date/epoch directory structure:
  {vad_root}/{patient}/encoding/{source_run}/{date}/{epoch}/{patient}_{date}_{epoch}_word_idx.npy

Having a shared bundle means per-epoch GLM weight vectors (W) all live in the
same feature space, making cosine-distance comparisons across epochs interpretable.

Usage:
    python encoding_singlepca_epoch_bundle_worker.py <patient> <vad_root> <out_dir>
        --source-run NAME          word_level_duration_cv_filtered_speech_per_epoch
...
def main():
    ...
    try:
        ...
        # ── Discover all epoch word-index files (nested date/epoch structure) ──
        ...
        epoch_entries = []   # list of (date_str, epoch_str, idx_path)
        for date_dir in sorted(enc_base.iterdir()):
            ...
            for epoch_dir in sorted(date_dir.iterdir()):
                ...
                if idx_files:
                    epoch_entries.append((date_dir.name, epoch_dir.name, idx_files[0]))

        if not epoch_entries:
            raise ValueError(f'No word-index files found under {enc_base}')

        print(f'Found {len(epoch_entries)} (date, epoch) pairs across '
              f'{len(set(d for d,_,_ in epoch_entries))} dates', flush=True)

        emb_all = np.load(emb_path, mmap_mode='r')
        cnt_all = np.load(cnt_path, mmap_mode='r')

        # ── Load and concatenate embeddings from all epochs ───────────────────
        X_chunks  = []
        n_total   = 0
        used_keys = []

        for date_str, epoch_str, idx_path in epoch_entries:
            ...
            if len(X_clean) > 0:
                ...
                n_total  += len(X_clean)
                used_keys.append(f'{date_str}/{epoch_str}')
            else:
                print(f'  [{date_str}/{epoch_str}] all NaN — skipped', flush=True)

        if not X_chunks:
            raise ValueError(f'No valid words found for {patient}')

        X_all = np.concatenate(X_chunks, axis=0)
        print(f'Total: {n_total} words from {len(used_keys)} epochs, shape {X_all.shape}',
              flush=True)

        # ── Fit global preprocessing bundle ───────────────────────────────────
        print(f'Fitting PCA({args.n_pca})...', flush=True)
