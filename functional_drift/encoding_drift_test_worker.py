"""
Encoding drift Phase-2 worker — cross-day inference and permutation testing.

For each test date > train_date, applies the Phase-1 preprocessing bundle and model
to test-day words, computes actual metrics, then runs N permutation tests (shuffle
training X, refit bundle, refit GLM, evaluate on same test set) parallelized via
joblib within the job.

Usage:
    python encoding_drift_test_worker.py <patient> <vad_root>
        --train-date DATE
        ...
        [--n-jobs INT]            joblib workers for permutations (default 4)
...
# ── Utilities (self-contained, mirrors poisson_glm_worker) ───────────────────
...
def fit_glm_cpu(X_np, Y_np, alpha, log_durs_np, max_iter=200, tol=1e-6):
    """CPU-only GLM fit for use inside joblib workers."""
    n, d = X_np.shape
    K = Y_np.shape[1]
    X_t = torch.as_tensor(X_np.astype(np.float32))
    Y_t = torch.as_tensor(Y_np.astype(np.float32))
    dur_t = torch.as_tensor(log_durs_np.astype(np.float32))
    model = PoissonRidgeBatched(d, K, alpha)  # CPU
...
# ── Top-level permutation worker (must be picklable by joblib) ────────────────
...
def _run_encoding_perm(
    ...
    bundle_perm = fit_preprocessing_bundle(X_train_shuf, n_pca=n_pca)
    X_tr_proc = apply_bundle(X_train_shuf, bundle_perm).astype(np.float32)
    X_te_proc = apply_bundle(X_test_raw, bundle_perm).astype(np.float32)

    W_perm, b_perm = fit_glm_cpu(X_tr_proc, Y_train, alpha, log_durs_train, max_iter=max_iter)

    eta = X_te_proc @ W_perm + b_perm + log_durs_test[:, None]
...
def main():
    ...
    try:
        ...
        bundle_pkl   = train_out / f'{patient}_fullday_bundle.pkl'
        train_idx_npy = train_out / f'{patient}_fullday_train_idx.npy'
        meta_json    = train_out / f'{patient}_fullday_meta.json'

        for f in [model_tar, bundle_pkl, train_idx_npy, meta_json]:
            if not f.exists():
                raise FileNotFoundError(f'Phase-1 output missing: {f}')
        ...
        # ── Load training features (for permutation test) ─────────────────────
        ...
        for test_date, test_idx_path in test_dates:
            ...
            try:
                ...
                # Null: duration-adjusted training mean rate (rate per unit time,
                # scaled by each test word's duration — same formula as poisson_glm_worker)
                ...
                # ── Permutation test (joblib parallel over permutations) ───────
                ...
                ll_perms = Parallel(n_jobs=args.n_jobs, backend='threading', verbose=0)(
                    delayed(_run_encoding_perm)(
                        ...
                        X_test_raw, Y_test, log_durs_test,
                        alpha, n_pca, 200,
                    )
                ...
                # p-value: fraction of perms >= actual (one-sided)
