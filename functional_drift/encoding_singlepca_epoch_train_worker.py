"""
Single-PCA epoch drift Phase-1 worker — fits a per-2-hour-epoch Poisson GLM
using a shared global PCA bundle (from Phase-0) and per-neuron alpha
hyperparameters from the existing nested-CV per-epoch results.

All per-epoch weight vectors are expressed in the same PCA feature space,
making cosine-distance comparisons across epochs and days interpretable.

Usage:
    python encoding_singlepca_epoch_train_worker.py <patient> <vad_root> <out_dir>
        --train-date DATE           e.g. 2024-04-01
        --train-epoch EPOCH         e.g. 17-19
        --cv-results-pkl PATH       {patient}_encoding_results_cv.pkl from per-epoch run
        --word-idx-path PATH        .npy of global word indices for this epoch
...
Outputs in <out_dir>/:
    {patient}_epoch_model.tar       {'W': Tensor(n_pca,K), 'b': Tensor(K,), 'alpha': Tensor(K,)}
    {patient}_epoch_train_idx.npy
    {patient}_epoch_meta.json
...
def main():
    ...
    parser.add_argument('--train-epoch', required=True,
                        help='Epoch string, e.g. 17-19')
    ...
    if success_path.exists():
        print(f'already done: {patient} {args.train_date}/{args.train_epoch}', flush=True)
        sys.exit(0)

    try:
        # ── Extract best alpha per neuron from epoch CV results ───────────────
        ...
        print(f'  n_neurons={len(best_alpha)}, '
              f'alpha=[{best_alpha.min():.3g}, {best_alpha.max():.3g}]', flush=True)
        ...
        print(f'  epoch word_idx: {len(word_idx)} words', flush=True)
        ...
        torch.save(
            ...
            out_dir / f'{patient}_epoch_model.tar',
        )
        np.save(out_dir / f'{patient}_epoch_train_idx.npy', train_global_idx)
        ...
        with open(out_dir / f'{patient}_epoch_meta.json', 'w') as f:
